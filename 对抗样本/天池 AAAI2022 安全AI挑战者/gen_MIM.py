...
data_loader = torch.utils.data.DataLoader(data_set, batch_size=CFG.batch_size, num_workers=4)
...
for steps, (input, label) in bar:
    ...
    # Adv_img, temp1, temp2 = fgsm(model_resnet, model_densenet, image=input, label=label)

    Adv_img,temp1,temp2= MIM(model_resnet, model_densenet, ord=np.inf, x=input, y=label)
    res_count += temp1
    dense_count += temp2
...
np.save('./MIM_data.npy', images)
np.save('./MIM_label.npy', labels)