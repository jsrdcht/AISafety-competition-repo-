...
class ToyModel(nn.Module):
    def __init__(self):
        ...
        self.debug=nn.Linear(84,1000000)
        self.fc3 = nn.Linear(1000000, 10)
...
def get_dataset():
    ...
    trainloader = torch.utils.data.DataLoader(my_trainset,
                                              batch_size=16, num_workers=8, sampler=train_sampler)
    return trainloader
...
for epoch in iterator:
    ...
    for data, label in trainloader:
        print('train_loader_length:',len(trainloader))
        ...
        for name, parms in model.named_parameters():
            if flag:
                print('-->index:', local_rank, ' -->grad_value:', parms.grad)
                print('-->name:', name,)
                flag=False
